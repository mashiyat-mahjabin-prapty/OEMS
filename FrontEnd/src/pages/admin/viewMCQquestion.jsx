...
const ViewMCQQuestionPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:2000/api/admin/courses/${courseID}/exams/${examID}`)
      .then((response) => {
  ...
  const handleOption1Change = (event) => { 
    setEditedQuestion({ ...editedQuestion, option1: event.target.value });
  };

  const handleOption2Change = (event) => {
    setEditedQuestion({ ...editedQuestion, option2: event.target.value });
  };

  const handleOption3Change = (event) => {
    setEditedQuestion({ ...editedQuestion, option3: event.target.value });
  };

  const handleOption4Change = (event) => {
    setEditedQuestion({ ...editedQuestion, option4: event.target.value });
  };
  ...
  const handleCorrectOptionChange = (optionIndex) => {
    const options = editedQuestion.options.map((option, index) => {
  ...
  const handleAnswerTextChange = (event) => {
    setEditedQuestion({ ...editedQuestion, answerText: event.target.value });
  };

  const handleSave = (questionID) => {
    console.log(courseID, examID, questionID);
    // Send updated question data to the backend
    axios.put(`http://localhost:2000/api/admin/courses/${courseID}/exams/${examID}/updateQuestions/${questionID}`, editedQuestion)
      .then(response => {
    ...
    <Container maxWidth="md">
      <Paper elevation={3} sx={{ padding: 3, marginTop: 3 }}>
        <Typography variant="h5" gutterBottom>
          {isEditing ? 'Edit MCQ Question' : 'View MCQ Question'}
        </Typography>
          ...
          <Grid item xs={8}>
            {selectedQuestion !== null && (
              <div>
                ...
                <div>
                  <Typography variant="subtitle1">Option 1:</Typography>
                  {isEditing ? (
                    <TextField
                      multiline
                      fullWidth
                    ...
                    <Typography variant="body1" sx={{ whiteSpace: 'pre-line' }}>
                      {questions.find((q) => q.question_id === selectedQuestion)?.option1}
                    </Typography>
                  )}
                </div>
                <div>
                  <Typography variant="subtitle1">Option 2:</Typography>
                  {isEditing ? (
                    <TextField
                      multiline
                      fullWidth
                    ...
                    <Typography variant="body1" sx={{ whiteSpace: 'pre-line' }}>
                      {questions.find((q) => q.question_id === selectedQuestion)?.option2}
                    </Typography>
                  )}
                </div>
                <div>
                  <Typography variant="subtitle1">Option 3:</Typography>
                  {isEditing ? (
                    <TextField
                      multiline
                      fullWidth
                    ...
                    <Typography variant="body1" sx={{ whiteSpace: 'pre-line' }}>
                      {questions.find((q) => q.question_id === selectedQuestion)?.option3}
                    </Typography>
                  )}
                </div>
                <div>
                  <Typography variant="subtitle1">Option 4:</Typography>
                  {isEditing ? (
                    <TextField
                      multiline
                      fullWidth
                    ...
                    <Typography variant="body1" sx={{ whiteSpace: 'pre-line' }}>
                      {questions.find((q) => q.question_id === selectedQuestion)?.option4}
                    </Typography>
                  )}
                </div>
                <div>
...
export default ViewMCQQuestionPage;
