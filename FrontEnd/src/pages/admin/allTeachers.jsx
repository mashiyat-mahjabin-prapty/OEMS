  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography
          sx={{ flex: '1 1 100%' , fontFamily: "Helvetica Neue", fontWeight:"bold" , marginTop:'20px'}}
          variant="h4"
          id="tableTitle"
          component="div"
        >
...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:2000/api/admin/teachers', requestOptions)
      .then(response => {
        ...
        setRows(response.data.teachers);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = rows.map((n) => n.teacher_id);
  ...
  const handleButtonClick = (teacher_id) => {
    console.log('Button clicked!');
    console.log('teacher_id:', teacher_id);
    window.location.href = `/teacher-details/${teacher_id}`;
  };
      ...
      <Paper sx={{ width: '100%', mb: 4 , display:'flex', flexDirection:'column' }}>
        ...
        <TableContainer>
          ...
          >
            ...
            <TableBody>
              {rows.map((row, index) => {
                const isItemSelected = isSelected(row.teacher_id);
                const labelId = `enhanced-table-checkbox-${index}`;

                return (
                  <TableRow
                    hover
                    onClick={(event) => handleClick(event, row.teacher_id)}
                    role="checkbox"
