...
//   console.log(`Adding exam to ${courseID}`);
//   window.location.href = `/teacher/courses/coursedetails`;
// };
...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:2000/api/teacher/courses/', requestOptions)
      .then(response => {
        //console.log('Response data:', response.data);
        console.log('data:', response.data.courses);

        const temp_rows = response.data.courses.map(course => ({
        ...
        // delete the empty rows of temp_rows
        for (let i = 0; i < temp_rows.length; i++) {
          if (temp_rows[i].name === '') {
            temp_rows.splice(i, 1);
          }
        }
        console.log('Temp rows:', temp_rows);
        setRows(temp_rows);
        setRender(false);
  ...
  const handleButtonClick = (course_id) => {
    ...
    window.location.href = `/teacher/courses/coursedetails/${course_id}`;
  };
              ...
              {filteredCourses.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">{row.Cls}</TableCell>
                    <TableCell align="left">{row.subject}</TableCell>
                    <TableCell align="left"><button onClick={()=>handleButtonClick(row.course_id)}>Course Details</button></TableCell>
                  </TableRow>
                );
              })}
              {/* {emptyRows > 0 && (
